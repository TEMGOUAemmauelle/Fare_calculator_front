...
export default function ErrorMessage({
  ...
  const ERROR_TYPES = {
    ...
    geolocation: {
      icon: MapPin,
      title: t('error.geolocation_title'),
      color: 'blue',
    ...
    validation: {
      icon: AlertCircle,
      title: t('error.validation_title'),
      color: 'orange',
  ...
  if (variant === 'banner') {
    return (
      <motion.div
        ...
        exit={{ opacity: 0, y: -20 }}
        className={`w-full bg-linear-to-r ${config.bgGradient} border-l-4 ${config.borderColor} p-4 ${className}`}
      >
  ...
  if (variant === 'inline') {
    return (
      ...
      >
        {showIcon && <Icon className={`w-4 h-4 ${config.iconColor} shrink-0`} />}
  ...
  if (variant === 'modal') {
    return (
      ...
      >
        ...
        >
          {/* Header avec icône */}
          <div className={`bg-linear-to-br ${config.bgGradient} px-6 py-8 text-center`}>
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
          ...
          <div className="p-6 flex gap-3">
            {onRetry && (
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={onRetry}
                className={`flex-1 py-3 bg-linear-to-r ${config.bgGradient} hover:opacity-90 border ${config.borderColor} rounded-xl font-bold text-gray-900 transition-all flex items-center justify-center gap-2`}
              >
  ...
  return (
    ...
    >
      ...
      <div className={`h-2 bg-linear-to-r ${config.bgGradient}`} />
        ...
        <div className="flex items-start gap-4">
          {showIcon && (
            <motion.div
              ...
              className={`p-3 ${config.iconBg} rounded-2xl shrink-0`}
            >
            ...
            <div className="flex items-center gap-3 mt-4">
              {onRetry && (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={onRetry}
                  className={`inline-flex items-center gap-2 px-5 py-2.5 bg-linear-to-r ${config.bgGradient} hover:opacity-90 border ${config.borderColor} rounded-xl font-bold text-gray-900 transition-all shadow-sm`}
                >
                  <RefreshCw className="w-4 h-4" />
                  {t('error.retry')}
                </motion.button>
