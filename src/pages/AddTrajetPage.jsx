...
/**
 * @fileoverview AddTrajetPage - Version corrigée
 * 
 * Fixes:
 * - Drawer starts higher (55vh)
 * - Switch more compact
 * - Stats overlay repositioned
 * - Cleaner layout
 */
    ...
    <div className="relative w-full h-screen overflow-hidden bg-white antialiased">
      ...
      <div className="absolute top-4 left-0 right-0 z-20 px-4 flex items-center justify-between">
          ...
          <div className="flex items-center p-0.5 bg-white/90 backdrop-blur-sm rounded-full shadow-md">
              <button onClick={() => navigate('/estimate')} className="px-3 py-1.5 text-gray-400 font-bold text-[9px] uppercase tracking-wide flex items-center gap-1 hover:text-gray-600">
          ...
          <Drawer.Content className="fixed bottom-0 left-0 right-0 z-50 flex flex-col bg-white rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] outline-none h-[55vh]">
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-6">
                <form onSubmit={handleSubmit} className="flex flex-col h-full">
                    <h2 className="text-lg font-bold tracking-tight text-[#141414] mb-4 italic">Partager un <span className="text-[#f3cd08]">tarif</span></h2>
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                         ...
                         <div className={`flex items-center gap-3 p-3 rounded-2xl border transition-all ${activeSearchField === 'arrivee' ? 'border-[#f3cd08] bg-white shadow-lg' : 'border-transparent bg-gray-50'}`}>
                            <MapPin className={`w-4 h-4 shrink-0 ${activeSearchField === 'arrivee' ? 'text-[#f3cd08]' : 'text-gray-400'}`} />
                            <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 p-3 rounded-2xl bg-[#f3cd08]/10 border border-[#f3cd08]/20">
                             ...
                             <div className="flex-1 min-w-0">
                                <span className="text-[8px] font-bold text-[#f3cd08] uppercase tracking-widest block">Prix payé</span>
                                <div className="flex items-baseline gap-2">
                                    <input 
                                        type="number" value={formData.price} onChange={(e) => setFormData({...formData, price: e.target.value})}
                                        placeholder="0" className="w-full text-lg font-bold text-[#141414] border-none p-0 focus:ring-0 bg-transparent placeholder:text-gray-200"
                                    />
                                    <span className="text-[10px] font-bold text-gray-400 shrink-0">FCFA</span>
                                </div>
                             </div>
                        </div>
                    </div>

                    <div className="flex-1 overflow-y-auto mt-4 min-h-[100px]">
                        <AnimatePresence mode="wait">
                            {activeSearchField ? (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-1">
                                    {isSearching ? <div className="p-3 text-[9px] font-bold text-gray-400 animate-pulse">Recherche...</div> : 
                                        suggestions.map((s, i) => (
                                ...
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="space-y-1.5">
                                            <p className="text-[8px] font-bold text-gray-300 uppercase tracking-wide">Météo</p>
                                            <div className="flex gap-2">
                                                {[{v:0, i:Sun}, {v:2, i:CloudRain}].map(o => (
                                                    <button key={o.v} type="button" onClick={() => setFormData({...formData, meteo: o.v})} className={`flex-1 p-2 rounded-xl border transition-all ${formData.meteo === o.v ? 'bg-[#141414] border-[#141414] text-white' : 'bg-gray-50 border-transparent text-gray-300'}`}>
                                        ...
                                        <div className="space-y-1.5">
                                            <p className="text-[8px] font-bold text-gray-300 uppercase tracking-wide">Période</p>
                                            <select value={formData.heure} onChange={(e) => setFormData({...formData, heure: e.target.value})} className="w-full bg-gray-50 border-none rounded-xl text-[10px] font-semibold p-2 outline-none uppercase text-gray-600">
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center">
                                            <p className="text-[8px] font-bold text-gray-300 uppercase tracking-wide">Qualité</p>
                                        ...
                                        />
                                        <div className="flex justify-between"><ThumbsDown className="w-3 h-3 text-gray-300"/><ThumbsUp className="w-3 h-3 text-gray-300"/></div>
                                    </div>
                                </motion.div>
                            )}
                        </AnimatePresence>
