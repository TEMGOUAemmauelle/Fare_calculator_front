    ...
    <div className="min-h-screen bg-white text-[#141414] font-sans antialiased pb-20">
         ...
         <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <h1 className="text-base font-black uppercase tracking-tighter italic leading-none">
                TRAJETS<span className="text-[#f39908]">COMMU</span>
              </h1>
              <div className="h-[2px] w-5 bg-[#f39908] mt-1" />
            </div>
         </div>
      ...
      <main className="px-6 mt-4 space-y-6">
        ...
        <div className="relative group">
           <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-300 group-focus-within:text-[#f39908] transition-colors" />
           <input 
              type="text" placeholder="Rechercher une destination..." 
              value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-3 bg-gray-50 border-none rounded-2xl text-[11px] font-bold text-gray-700 placeholder:text-gray-300 focus:ring-1 focus:ring-[#f39908]/30 outline-none transition-all"
           />
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            ...
            <span className="text-[8px] font-black text-[#f39908] uppercase tracking-widest">{filteredTrajets.length} Trajets</span>
          </div>
              ...
              <AnimatePresence>
                {filteredTrajets.map((trajet, idx) => (
                  <motion.div
                    ...
                    className="relative bg-white p-3.5 rounded-2xl border border-gray-100 shadow-[0_2px_8px_-4px_rgba(0,0,0,0.03)] hover:border-[#f39908]/30 transition-all duration-300"
                  >
                    <div className="flex items-center justify-between gap-4">
                       <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2">
                             <Navigation className="w-2.5 h-2.5 text-[#f39908] shrink-0 transform rotate-45" />
...
function MiniBadge({ icon: Icon, label, highlight }) {
  ...
  return (
    <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-md ${highlight ? 'bg-black text-white' : 'bg-gray-50 text-gray-400'}`}>
      <Icon className={`w-2.5 h-2.5 ${highlight ? 'text-[#f39908]' : 'text-gray-300'}`} />
