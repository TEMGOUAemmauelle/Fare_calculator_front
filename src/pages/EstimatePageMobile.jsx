...
/**
 * @fileoverview EstimatePageMobile - Version corrigée
 * 
 * Fixes:
 * - Drawer starts higher (55vh)
 * - Switch more compact and repositioned
 * - Stats overlay repositioned (no overlap)
 * - Input focus doesn't collapse drawer
 */
    ...
    <div className="relative w-full h-screen overflow-hidden bg-white antialiased">
      ...
      <div className="absolute top-4 left-0 right-0 z-20 px-4 flex items-center justify-between">
          ...
          <div className="flex items-center p-0.5 bg-white/90 backdrop-blur-sm rounded-full shadow-md">
              ...
              <button onClick={() => navigate('/add-trajet')} className="px-3 py-1.5 text-gray-400 font-bold text-[9px] uppercase tracking-wide flex items-center gap-1 hover:text-gray-600">
                  <PlusCircle className="w-3 h-3" /> Contribuer
              </button>
          </div>

          <div className="flex gap-2">
             <button onClick={() => navigate('/stats')} className="p-2.5 bg-white rounded-xl text-gray-400 hover:text-[#f3cd08] shadow-md"><BarChart2 className="w-4 h-4" /></button>
             <button onClick={() => navigate('/trajets')} className="p-2.5 bg-white rounded-xl text-gray-400 hover:text-[#f3cd08] shadow-md"><Globe className="w-4 h-4" /></button>
          </div>
      </div>

      {/* ROUTE STATS - Bottom Left (no overlap) */}
      <AnimatePresence>
      ...
      {/* DRAWER - Starts at 55% height */}
      <Drawer.Root open={true} modal={false} dismissible={false}>
        <Drawer.Portal>
            <Drawer.Content className="fixed bottom-0 left-0 right-0 z-50 flex flex-col bg-white rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] outline-none h-[55vh]">
                ...
                <div className="flex-1 overflow-y-auto px-6 pb-6">
                    {!prediction ? (
                        <div className="flex flex-col h-full">
                            <h2 className="text-lg font-bold tracking-tight text-[#141414] mb-4 italic">Estimation du <span className="text-[#f3cd08]">prix</span></h2>
                            ...
                            <div className="flex flex-col gap-3 shrink-0">
                                <div className={`flex items-center gap-3 p-3 rounded-2xl border transition-all ${activeSearchField === 'depart' ? 'border-[#f3cd08] bg-white shadow-lg' : 'border-transparent bg-gray-50'}`}>
                                    <Navigation className={`w-4 h-4 shrink-0 ${activeSearchField === 'depart' ? 'text-[#f3cd08]' : 'text-gray-400'}`} />
                                    <div className="flex-1 min-w-0">
                                        <span className="text-[8px] font-bold text-gray-400 uppercase tracking-widest block">Départ</span>
                                        <SearchBarEnhanced
                                            ...
                                            className="w-full text-sm font-semibold text-gray-800 border-none p-0 focus:ring-0 bg-transparent placeholder:text-gray-300 truncate"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className={`flex items-center gap-3 p-3 rounded-2xl border transition-all ${activeSearchField === 'arrivee' ? 'border-[#f3cd08] bg-white shadow-lg' : 'border-gray-100 bg-white'}`}>
                                    <MapPin className={`w-4 h-4 shrink-0 ${activeSearchField === 'arrivee' ? 'text-[#f3cd08]' : 'text-[#141414]'}`} />
                                    <div className="flex-1 min-w-0">
                                        <span className="text-[8px] font-bold text-gray-400 uppercase tracking-widest block">Destination</span>
                                        <SearchBarEnhanced
                                            ...
                                            className="w-full text-base font-bold text-[#141414] border-none p-0 focus:ring-0 bg-transparent placeholder:text-gray-200 truncate"
                                        />
                                    </div>
                            ...
                            <div className="flex-1 overflow-y-auto mt-4 min-h-[120px]">
                                <AnimatePresence mode="wait">
                                    {(activeSearchField && isSearching) ? (
                                       <motion.div key="skeleton" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><SuggestionSkeleton /></motion.div>
                                    ) : (activeSearchField && suggestions.length > 0) ? (
                                        <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-1 pb-4">
                                            {suggestions.map((s, i) => (
                                                <button key={i} onMouseDown={() => handleSelectSuggestion(s)} className="w-full p-3 rounded-xl flex items-center gap-3 text-left group hover:bg-gray-50 transition-all">
                                                    ...
                                                    <div className="flex flex-col min-w-0">
                                                        ...
                                                        <span className="text-[9px] text-gray-400 font-medium uppercase truncate">{s.place_formatted}</span>
                                                    </div>
                                                </button>
                                            ))}
                                        </motion.div>
                                        ...
                                        <motion.div key="options" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-2">
                                                    <p className="text-[9px] font-bold text-gray-300 uppercase tracking-wide">Météo</p>
                                                    <div className="flex gap-2">
                                                        {WEATHER_OPTIONS.map(opt => (
                                                            <button key={opt.value} onClick={() => setMeteo(opt.value)} className={`flex-1 p-2.5 rounded-xl border transition-all ${meteo === opt.value ? 'bg-[#141414] border-[#141414] text-white' : 'bg-gray-50 border-transparent text-gray-300'}`}>
                                                ...
                                                <div className="space-y-2">
                                                    <p className="text-[9px] font-bold text-gray-300 uppercase tracking-wide">Période</p>
                                                    <select value={heureTrajet} onChange={(e) => setHeureTrajet(e.target.value)} className="w-full bg-gray-50 border-none rounded-xl text-[10px] font-semibold p-2.5 outline-none uppercase text-gray-600">
                            ...
                            {departPlace && arriveePlace && !activeSearchField && (
                        ...
                        <div className="py-4">
                             <div className="flex justify-between items-center mb-6">
                                ...
                                <button onClick={() => setPrediction(null)} className="px-3 py-1.5 bg-gray-100 rounded-xl text-[9px] font-bold text-gray-500 uppercase hover:bg-[#f3cd08] hover:text-black transition-all">Nouvelle</button>
                             </div>
